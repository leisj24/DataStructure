...
double calcASL_Fail_HT1(HashTable1 *ht) {
    ...
    // 对于每个可能的哈希地址，计算查找失败需要的比较次数
    for (int i = 0; i < ht->p; i++) {
        int j = 0;
        while (j < ht->size && ht->data[(i + j) % ht->size] != EMPTY) {
            j++;
        }
...
double calcASL_Fail_HT2(HashTable2 *ht) {
    ...
    for (int i = 0; i < ht->p; i++) {
        ...
        totalComparisons += len;  // 查找失败需要比较len次
    }
